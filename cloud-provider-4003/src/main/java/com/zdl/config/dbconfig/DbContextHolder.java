...
public class DbContextHolder {
    ...
    public static void setCurrentDsStr(String dsName) {
        if (StringUtils.isBlank(dsName)) {
            System.out.println("==========>dbType is null,throw NullPointerException");
            throw new NullPointerException();
        }
        if (!dataSources.containsKey(dsName)) {
            System.out.println(String.format("==========>datasource not exists,dsName=%s", dsName));
    ...
    public static void addDataSource(String dsName, DataSource dataSource) {
        if (dataSources.containsKey(dsName)) {
            System.out.println(String.format("==========>dataSource=%s already exist", dsName));
    ...
    public static DataSource getDefaultDataSource() {
        if (StringUtils.isBlank(defaultDs)) {
            System.out.println("==========>default datasource must be configured");
            throw new RuntimeException("default datasource must be configured.");
        }
        if (!dataSources.containsKey(defaultDs)) {
            System.out.println("==========>The default datasource must be included in the datasources");
